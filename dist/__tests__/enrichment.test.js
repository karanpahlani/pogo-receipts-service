...
describe('Enrichment Service Tests', () => {
    ...
    describe('standardizeBrand function', () => {
        it('should handle empty or null brand', () => {
            ...
            expect(standardizeBrand(null)).toBe('unknown');
            expect(standardizeBrand(undefined)).toBe('unknown');
        });
    ...
    describe('enrichReceiptData function', () => {
        ...
        it('should successfully enrich receipt data with valid AI response', async () => {
            ...
            const result = await enrichReceiptData('iPhone 15 Pro Max 256GB Natural Titanium', 'Apple Store', 'Apple');
            expect(result).toEqual({
                brand: 'Apple',
                category: ['Electronics', 'Mobile Phones', 'Smartphones'],
                confidence: 'high'
            });
            expect(mockGenerateLLMText).toHaveBeenCalledWith(expect.stringContaining('iPhone 15 Pro Max 256GB Natural Titanium'), {
                temperature: 0.1,
                maxTokens: 200,
                model: 'gpt-4o-mini'
            });
        ...
        it('should handle AI response wrapped in markdown code blocks', async () => {
            ...
            const result = await enrichReceiptData('Samsung Galaxy S24 Ultra', 'Best Buy', 'Samsung');
        ...
        it('should handle AI response with just code block markers', async () => {
            ...
            const result = await enrichReceiptData('MacBook Pro 16 inch', 'Apple Store');
            expect(result.brand).toBe('Apple');
        ...
        it('should fallback to unknown values when AI service fails', async () => {
            ...
            const result = await enrichReceiptData('Unknown Product', 'Unknown Store', 'Known Brand');
            expect(result).toEqual({
                brand: 'Known Brand',
                category: ['unknown'],
                confidence: 'low'
        ...
        it('should fallback when AI returns invalid JSON', async () => {
            ...
            const result = await enrichReceiptData('Test Product', 'Test Store');
            expect(result).toEqual({
                brand: 'unknown',
                category: ['unknown'],
                confidence: 'low'
        ...
        it('should fallback when AI response fails validation', async () => {
            ...
            const result = await enrichReceiptData('Test Product', 'Test Store', 'Fallback Brand');
            expect(result).toEqual({
        ...
        it('should include all required fields in prompt', async () => {
            mockGenerateLLMText.mockResolvedValue(mockValidResponse);
            await enrichReceiptData('Nintendo Switch OLED Console', 'GameStop', 'Nintendo');
        ...
        it('should handle missing existing brand gracefully', async () => {
            mockGenerateLLMText.mockResolvedValue(mockValidResponse);
            await enrichReceiptData('Generic Product Description', 'Generic Store');
            const callArgs = mockGenerateLLMText.mock.calls[0];
            const prompt = callArgs[0];
        ...
        it('should handle edge cases with special characters', async () => {
            ...
            const result = await enrichReceiptData('L\'Oréal Paris Voluminous Mascara', 'CVS Pharmacy', 'L\'Oréal');
        ...
        it('should handle unicode characters in response', async () => {
            ...
            const result = await enrichReceiptData('Häagen-Dazs Vanilla Ice Cream', 'Whole Foods', 'Häagen-Dazs');
        ...
        it('should handle long product descriptions', async () => {
            ...
            await enrichReceiptData(longDescription, 'Test Store', 'Test Brand');
        ...
        it('should use correct AI model parameters', async () => {
            mockGenerateLLMText.mockResolvedValue(mockValidResponse);
            await enrichReceiptData('Test Product', 'Test Store', 'Test Brand');
            expect(mockGenerateLLMText).toHaveBeenCalledWith(expect.any(String), {
                temperature: 0.1,
                maxTokens: 200,
                model: 'gpt-4o-mini'
            });
        ...
        it('should handle network timeouts gracefully', async () => {
            ...
            const result = await enrichReceiptData('Timeout Test Product', 'Timeout Store', 'Timeout Brand');
            expect(result).toEqual({
                brand: 'Timeout Brand',
                category: ['unknown'],
                confidence: 'low'
