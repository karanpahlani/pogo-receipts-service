...
describe('Database Integration Tests', () => {
    ...
    describe('Receipt Schema Operations', () => {
        ...
        it('should enforce primary key constraint', async () => {
            ...
            await expect(testDb.insert(receipts).values({
                ...testReceiptData,
    ...
    describe('Database Connection', () => {
        ...
        it('should handle database queries with parameters', async () => {
            ...
            const result = await testDb.execute(sql `SELECT receipt_id FROM receipts WHERE merchant_name = ${merchantName} AND receipt_id LIKE 'test-%'`);
            expect(result.rows).toHaveLength(1);
            expect(result.rows[0].receipt_id).toBe('test-query-param');
        });
    });
    describe('Query Performance', () => {
        const perfTestReceiptData = {
        ...
        describe('Database Query Mocking', () => {
            ...
            it('should mock database select operations', async () => {
                const mockSelect = {
                    ...
                    limit: jest.fn().mockResolvedValue([validReceiptData])
                };
